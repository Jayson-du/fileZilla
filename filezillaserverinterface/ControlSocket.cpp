...
bool CControlSocket::GetCommand(CStdString &command, CStdString &args)
{
	...
	if (!str2.Left(5).CompareNoCase(_T("PASS "))) {
		if (m_owner.m_pOptions->GetOptionVal(OPTION_LOGSHOWPASS)) {
			...
			CStdString msg = str2;
			for (size_t i = 5; i < msg.GetLength(); ++i) {
				//msg[i] = L'*';
				msg.AppendChar('*');
			}
...
t_command const& CControlSocket::MapCommand(CStdString const& command, CStdString const& args)
{
	...
	if( it != command_map.end() ) {
		...
		if( it->second.bHasargs && args.IsEmpty() ) {
...
BOOL CControlSocket::DoUserLogin(LPCTSTR password)
{
	...
	if (m_status.user.GetIpLimit() && count >= m_status.user.GetIpLimit()) {
		CStdString str;
		if (count==1)
			str.Format(_T("Refusing connection. Reason: No more connections allowed from this IP. (%s already connected once)"), peerIP.GetString());
		else
			str.Format(_T("Refusing connection. Reason: No more connections allowed from this IP. (%s already connected %d times)"), peerIP.GetString(), count);
		SendStatus(str, 1);
...
bool CControlSocket::CreatePassiveTransferSocket()
{
	...
	PasvPortRandomizer randomizer(pasvPortManager, peerIP.GetString(), *m_owner.m_pOptions);
	while (retries > 0) {
