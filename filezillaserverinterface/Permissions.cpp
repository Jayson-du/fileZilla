...
void CPermissions::AddFactsListingEntry(std::list<t_dirlisting> &result, bool isDir, const char* name, const t_directory& directory, __int64 size, FILETIME* pTime, const char*, bool *enabledFacts)
{
	...
	if (!enabledFacts || enabledFacts[2]) {
		if (mtime.dwHighDateTime || mtime.dwLowDateTime) {
			...
			memcpy(result.back().buffer + result.back().len, str.GetString(), str.GetLength());
									  ...
									  bool *enabledFacts /*=0*/)
{
	...
	if (dirToDisplayUTF8.empty() && !dirToDisplay.IsEmpty())
		return PERMISSION_DENIED;
...
bool CPermissions::CheckUserLogin(CUser const& user, LPCTSTR pass, BOOL noPasswordCheck /*=FALSE*/)
{
	...
	if (user.password.IsEmpty() || noPasswordCheck) {
		return true;
	}
	...
	if (user.salt.IsEmpty()) {
		...
		if (user.password.GetLength() != MD5_HEX_FORM_LENGTH) {
			// But it isn't...
			return false;
		}
	...
	CStdString hash = ssl.SHA512(reinterpret_cast<unsigned char const*>(saltedPassword.c_str()), saltedPassword.size()).c_str();

	return !hash.IsEmpty() && user.password == hash;
}
...
{
	//TiXmlElement *pXML = /*COptions::GetXML()*/NULL;
	...
	//	if (!user.password.IsEmpty() && user.salt.IsEmpty() && user.password.GetLength() != MD5_HEX_FORM_LENGTH) {
	//		user.generateSalt();
